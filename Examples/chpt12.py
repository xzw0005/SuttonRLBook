...
# And we use exact 19 groups, so the values for each group is exact the value for that state
class ValueFunction:
...
class TemporalDifferenceLambda(ValueFunction):
    ...
    def learn(self, state, reward):
        # update the eligibility trace and values
...
class TrueOnlineTemporalDifferenceLambda(ValueFunction):
    ...
    def learn(self, state, reward):
        ...
        self.values += self.stepSize * (delta + lastStateValue - self.oldStateValue) * self.eligibility
        self.values[self.lastState] -= self.stepSize * (lastStateValue - self.oldStateValue)
